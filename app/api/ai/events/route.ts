...
export async function POST(req: Request): Promise<Response> {
  try {
    ...
    const limit = rateLimit(`ai:events:${user.id}`, { limit: 5, windowMs: 60000 });
    if (!limit.ok) {
      return jsonError(
        429,
    ...
    const body = await parseBody(req, eventsRequestSchema);

    const result = await generateJson<unknown>({
      system: CULTURAL_EXPERT_SYSTEM,
      prompt: buildEventsPrompt(body),
      schema: eventsGeminiSchema,
    });

    // Never patch or substitute model output — an off-shape response is an
    // honest 502, not fabricated content.
    const parsed = eventsResultSchema.safeParse(result.data);
    if (!parsed.success) {
      return jsonError(
        502,
